...
// Finds the slope/grade/incline angle of ground underneath a PlayerController
public class GroundChecker : MonoBehaviour {
    ...
    void Awake()
    {
        ...
        if (controller == null) { Debug.LogError("GroundChecker did not find a PlayerController component."); }
    }
    ...
    void FixedUpdate()
    {
        ...
        if (controller && controller.isGrounded) {
            CheckGround(new Vector3(transform.position.x, transform.position.y - (GetComponent<CapsuleCollider2D>().size.y / 2) + startDistanceFromBottom, transform.position.z));
        }
    }
    ...
    void OnDrawGizmosSelected()
    {
        if (showDebug)
        {
            ...
            Vector3 startPoint = new Vector3(transform.position.x, transform.position.y - (GetComponent<CapsuleCollider2D>().size.y  / 2) + startDistanceFromBottom, transform.position.z);
            Vector3 endPoint = new Vector3(transform.position.x, transform.position.y - (GetComponent<CapsuleCollider2D>().size.y  / 2) + startDistanceFromBottom - sphereCastDistance, transform.position.z);
